...
Updated: No candidate limit, No top_k in UI.
Automatically fetches ALL matching tenders and adjusts response based on user query.
"""
import re
import os
import json
...
TOP_K_INTERNAL = 577          # ALWAYS used internally for chat
...
def get_distinct_values(col):
    try:
        resp = supabase.table("GEM_SERVICE_TABLE").select(f'"{col}"').execute()
...
def fetch_all_rows(filters):
    ...
    query = supabase.table("GEM_SERVICE_TABLE").select("*")
...
def in_range(value, min_val, max_val):
    if value is None:
        return True  # don't reject row if numeric field missing
...
def apply_advanced_filters(rows, filters):
    ...
    # tokens that we will treat as "missing" or "not found"
    # MISSING_TOKENS = {"Not found", "notavailable", "not available", "na", "n/a", "-", "none", "not provided", "tbd"}
    MISSING_TOKENS = {"not found", "notavailable", "not available", "na", "n/a", "-", "none", "not provided", "tbd"}
    ...
    for r in rows:
        ...
        # If user requested to remove rows whose Estimated Bid Value is "not found" (or similar),
        # skip them here.
        # if filters.get("remove_est_notfound"):
        #     est_raw = r.get("Estimated Bid Value")
        #     est_norm = norm(est_raw).replace(" ", "")  # remove spaces for tokens like "not available"
        #     if est_norm in {t.replace(" ", "") for t in MISSING_TOKENS}:
        #         # skip this row
        #         continue
        if filters.get("remove_est_notfound"):
            est_raw = str(r.get("Estimated Bid Value", "")).strip().lower()

    # If no digits exist → treat as not found
        ...
        # ------------ MULTISELECT FILTERS ------------
        passed = True
        for field in ["Ministry Name", "Department Name", "Organisation Name", "Office Name", "Item Category"]:
        ...
        checks = [
            ...
            ("Estimated Bid Value", "est_min", "est_max", True),  # special case
        ...
        for col, min_key, max_key, is_est_val in checks:
            ...
            # SPECIAL CASE: Estimated Bid Value (varchar with numbers or "Not found")
            if is_est_val:
                # If user gave min/max, then "Not found" rows should be dropped
                if (min_v is not None or max_v is not None):
                    if str(raw_val).strip().lower() == "not found":
                        passed = False
                        break

            # Convert numeric text (e.g., "2,00,000" → 200000)
            val = to_int(raw_val)
...
with st.sidebar:
    ...
    # Checkbox to remove rows with Estimated Bid Value = "Not found" (or similar)
    remove_est_notfound = st.checkbox("Remove tenders with Estimated Bid Value = Not found", value=False)

    # Range numeric fields (varchar but treated as numbers)
...
if st.button("🔍 Load Tender Context"):
    with st.spinner("Fetching ALL matching tenders..."):
        # rows = fetch_all_rows(filters)
        # Fetch everything from DB
        raw_rows = supabase.table("GEM_SERVICE_TABLE").select("*").execute().data

        # Apply advanced python-side filters
    ...
    if not valid_rows:
        ...
        for i, row in enumerate(top_rows, start=1):
            ...
            context_blocks.append(f"--- Tender #{i} (Bid_No: {row.get('Bid_No')}, Sim={row['_similarity']:.3f}) ---\n{ctx}")

        st.session_state.context_text = "\n\n".join(context_blocks)
        st.success(f"Loaded context from {len(top_rows)} tenders (All {TOP_K_INTERNAL}). Start chatting!")
...
if st.session_state.context_text:
    ...
    user_query = st.chat_input("Ask something about these tenders...")
    ...
    if user_query:
        ...
        # Check for user asking for full data
        keywords = ["all tenders", "all rows", "full data", "complete data", "show everything", "table", "list"]

        if any(k in user_query.lower() for k in keywords):
            st.write("### 📄 Full List of Matching Tenders")
            st.dataframe(st.session_state.all_rows)
        else:
            # Normal chat through LLM
            system_prompt = (
                "You are a helpful tender assistant. "
                "You must answer only using the tender context provided. "
                "If an answer is unavailable, say so."
            )

...
def apply_advanced_filters(rows, filters):
    ...
    for r in rows:
        ...
        for field in ["Ministry Name", "Department Name",
...
with st.sidebar:
    ...
    end_date_range = st.date_input("End Date Range", [],help="Both FROM and TO dates are required.")
...
if st.session_state.context_text:
    ...
    if user_query:
        ...
        if any(k in user_query.lower() for k in keywords):
            # Lightweight assistant message in chat
            with st.chat_message("assistant"):
                st.markdown("Showing full list of matching tenders below.")
